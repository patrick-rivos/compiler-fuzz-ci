...
def find_unique_errors(root_dir):
    ...
    for dirpath, _, filenames in os.walk(root_dir):
        if "error-type.txt" in filenames:
            log_file = os.path.join(dirpath, "error-type.txt")
...
def move_directories_to_new_dir(target_dir, destination_dir, directories_to_move):
    ...
    for dir_name in directories_to_move:
        ...
        if os.path.isdir(source_path):
            ...
            dir = os.path.basename(source_path)
            try:
                shutil.copytree(source_path, os.path.join(target_path, f"{parent_dir}-{dir}"))

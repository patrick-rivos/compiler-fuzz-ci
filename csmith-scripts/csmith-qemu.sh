...
do
  ...
  # Compile for rv64gc
  if $(cat $script_location/compiler.path) -I$(cat $script_location/csmith.path)/include -march=rv64gc -mabi=lp64d -O3 $csmith_tmp/out.c -o $csmith_tmp/rv64gc.out 2>&1 | grep "internal compiler error";
  then
  ...
  QEMU_CPU="$($(cat $script_location/scripts.path)/march-to-cpu-opt --get-riscv-tag $csmith_tmp/rv64gc.out)" timeout -k 0.1 1 $(cat $script_location/qemu.path) $csmith_tmp/rv64gc.out > $csmith_tmp/rv64gc-qemu.log
  echo $? > $csmith_tmp/rv64gc-ex.log

  # Ensure both finished executing successfully (no timeouts/segfaults/etc)
  if [[ $(cat $csmith_tmp/user-config-ex.log) -eq 0 && $(cat $csmith_tmp/rv64gc-ex.log) -eq 0 ]];
  then
    # Check to see if the runtime hash differs
    if [[ $(diff $csmith_tmp/rv64gc-qemu.log $csmith_tmp/user-config-qemu.log | wc -l) -ne 0 ]];
    then
      ...
      cp $csmith_tmp/rv64gc-qemu.log $invocation_location/csmith-discoveries/$1-$COUNTER-qemu-diff-gc.c
    fi
  fi
